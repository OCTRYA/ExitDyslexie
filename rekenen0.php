...
<?php
    if(isset($_SESSION["exerciseManager"])&& isset($_POST['submitRekenen0'])){
    	...
    	for($i =0; $i< $exerciseManager->getAmountOfExercises();$i++){
    	    echo '<div class="row" id="calculateExercise' . $i . '" ><div class="col-sm-2"><p style="float:right"><span>'.showCorrectnessExercise($exerciseManager->getSingleExercises()[$i]->isResultCorrect()).'</span>&nbsp;&nbsp;' . $exerciseManager->getSingleExercises()[$i]->getFigure();
    	    echo " + ";
        ...
        for($i =0; $i< $exerciseManager->getAmountOfExercises();$i++){
            ...
            $addition->createSimpleExercise($exerciseManager->getLimitation());
    ...
    function showCorrectnessExercise($boolean){
        if($boolean){
            //echo 'ja';
